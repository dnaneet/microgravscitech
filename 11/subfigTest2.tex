...
However, when the DCM evaporates in Earth's gravity, the film is resistant to long wave type instabilities as seen by the blank DFT plot in figure \subref{fig:2a}. Hence a DCM film that is resistant to thermocapillary rupture in Earth's gravitational field is destabilized in zero gravity and is driven to rupture. Another interesting observation is that in Earth's gravity, the complete depletion of the liquid film takes place in about 1 minute. However, in a zero gravity environment, the film ruptures in around 4 seconds.
...
A positive growth rate ($\omega$) signifies a positive growth rate of long wave instabilities. Positive destabilizing growth rates occur in the case depicted in figure \subref{fig:2a}. A zero or negative growth rate signifies a damping of long wave instabilities. Negative, damping of instabilities occurs in figure \subref{fig:2b}.
...
%Effect of initial conditions on square domain
  \begin{figure} 
   \centering
    ...
    \subfloat[]{\includegraphics[width=0.45\linewidth]{Fig12}\label{fig:2c}}
...
%    \subfloat{\includegraphics[width=0.4\linewidth]{Fig20}\label{fig:3d}}
   \caption{The effect of the initial conditions in figure \ref{fig_ic} on film dynamics for a square domain with $L=2.238\lambda_\text{max}$ in figure \subref{fig:3a} and with a rectangular domain with $L_x = 2.238 \lambda_\text{max}, L_y=1.641 \lambda_\text{max}$ in figure \subref{fig:3b}. The discrete Fourier transforms show a cascade of frequencies. The dominant mode is that of the fastest growing wavelength.}
